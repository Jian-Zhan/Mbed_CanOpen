...
unsigned char digital_output_handler(CO_Data* d, unsigned char *newOutput, unsigned char size)
{
  ...
  /* fixed warning due to data size mismatch - not much of an issue since the 
  		data is last on the stack unless another function was called... */
  ...
  for (i=0; i < loops; i++)
  {
    /* warning because varsize is 8bits and the parameter is a pointer (32bit) 
		used for address to return a value */
